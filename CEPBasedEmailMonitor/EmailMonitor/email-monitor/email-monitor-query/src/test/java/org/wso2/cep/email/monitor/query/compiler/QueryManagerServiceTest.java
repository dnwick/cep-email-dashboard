   ...
   public  void testfilterQuerywithTo() throws EmailMonitorCompilerException {
     queryManagerService = new QueryManagerService();
      String[] result = queryManagerService.getSiddhiQuery(" to : (abc@wso2.com ) add label importnant" );
       ...
       Assert.assertEquals("from gmailInputStream select  threadID,labels, email:getAll(to) as to ,email:getAll(sender) as senders, \"importnant\" as newLabel group by threadID insert into filteredEmailDetails;",result[0]);
    ...
    public  void testfilterQuerywithfrom() throws EmailMonitorCompilerException {
        queryManagerService = new QueryManagerService();
        String[] result = queryManagerService.getSiddhiQuery(" from : (abc-dev@wso2.com ) add label importnant" );
        Assert.assertEquals(2,result.length);
        Assert.assertEquals("from gmailInputStream select  threadID,labels, email:getAll(to) as to ,email:getAll(sender) as senders, \"importnant\" as newLabel group by threadID insert into filteredEmailDetails;",result[0]);
        Assert.assertEquals("from filteredEmailDetails[(senders contains \"abc-dev@wso2.com\")] select threadID, newLabel as label insert into gmailOutputStream;",result[1]);
    }
