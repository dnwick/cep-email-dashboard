...
public final class GmailUtils {
	...
	public static void storeMailListInResponse(Message[] messagesArray,
		...
		for (int i = messagesArray.length - 1; i >= 0; i--) {
			...
			if (message != null) {
				String[] appliedLabels = ((IMAPMessage) message).getGoogleMessageLabels();
				boolean storedMail=false;
				for(int cnt=0;cnt<appliedLabels.length;cnt++){
					
					String label="";
					
					if(appliedLabels[cnt].substring(0,1).equals("\\")){
						label=appliedLabels[cnt].substring(1);
					}
					else{
						label=appliedLabels[cnt];
					}
					
					if(appliedLabels.length>1){
						
						if(label.equals("Starred") || label.equals("Important") || label.equals("Inbox")){
							
							if(cnt==(appliedLabels.length-1)){
	...
	public static int validateAccessToken(String accessToken)throws Exception{
		...
		response=sendPost(GmailConstants.GMAIL_OAUTH_VALIDATION_URL, nameValuePairs);
	...
	public static void retrieveNewAccessToken(MessageContext messageContext)throws Exception{
		...
		nameValuePairs.add(new BasicNameValuePair("client_id",messageContext.getProperty(GmailConstants.GMAIL_OAUTH_CLIENT_ID).toString()));
		nameValuePairs.add(new BasicNameValuePair("client_secret", messageContext.getProperty(GmailConstants.GMAIL_OAUTH_CLIENT_SECRET).toString()));
		nameValuePairs.add(new BasicNameValuePair("refresh_token", messageContext.getProperty(GmailConstants.GMAIL_OAUTH_REFRESH_TOKEN).toString()));
		nameValuePairs.add(new BasicNameValuePair("grant_type", GmailConstants.GMAIL_OAUTH_GRANT_TYPE));
		...
		axis2mc.getConfigurationContext().setProperty(GmailConstants.GMAIL_OAUTH_ACCESS_TOKEN,
				(obj.get("access_token")).toString());
		axis2mc.getConfigurationContext().setProperty(GmailConstants.GMAIL_OAUTH_USERNAME,
